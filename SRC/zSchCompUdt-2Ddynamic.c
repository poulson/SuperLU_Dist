...
if ( msg0 && msg2 ) {  /* L(:,k) and U(k,:) are not empty. */
     ...
     */
     ...
     int LDlookAhead_LBuff = lookAheadFullRow[lookAheadBlk-1];
     ...
     int LDRemain_LBuff = Remain_info[RemainBlk-1].FullRow;
#ifdef _OPENMP
#pragma omp parallel for 
#endif
...
#pragma omp parallel for private(j,iukp,rukp,tempu, jb, nsupc,ljb,segsize,\
    ...
    int Lnbrow   = lookAheadFullRow[lookAheadBlk-1];
    int Rnbrow   = Remain_info[RemainBlk-1].FullRow;
...
#pragma omp parallel for default (shared) \
     ...
     ***************************************************************/
    Rnbrow  = Remain_info[RemainBlk-1].FullRow;
    ncols   = Ublock_info[jj_cpu-1].full_u_cols;
